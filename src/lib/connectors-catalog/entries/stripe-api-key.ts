 ...
 * use the official hosted MCP (see ./stripe.ts). Backed by the built-in
 * `STRIPE` connector, which calls the Stripe SDK with a (preferably restricted)
 * API key. Sorted after the MCP entry so the hosted MCP stays the default.
 */
