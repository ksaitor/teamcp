...
export default async function ConnectorDetailPage({
  ...
  return (
    <div>
      ...
      {isExternalMcp && (connector.status === 'PENDING' || connector.status === 'ERROR') && (
        <ReauthBanner connectorId={connector.id} status={connector.status} authMode={config.authMode} />
      )}

      {xeroNeedsOrgPick && <XeroTenantPicker connectorId={connector.id} orgs={xeroOrgs} />}
      ...
      <div className='mt-6 rounded-md border border-border bg-card p-4'>
        ...
        <pre className='mt-2 text-xs text-muted-foreground'>{JSON.stringify(connector.config, null, 2)}</pre>
        <p className='mt-2 text-xs text-muted-foreground'>Credentials are encrypted and not displayed.</p>
      </div>
    </div>
  )
}
